...
public class GetCreditDayAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private ArrayList<GetCreditDay> data;
    private MyItemClickListener myItemClickListener;

    private int select = -1;

    public GetCreditDayAdapter(ArrayList<GetCreditDay> data, MyItemClickListener myItemClickListener) {
        this.data = data;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ViewHolder) {
            ...
            ((ViewHolder) holder).text.setText(data.get(position).getDayNumber()+"天");
        }
    }
